...
def compact(row: dict, external_observations: list[dict] | None = None) -> dict:
    ...
    if website.get("status") == "available" and identity_assessment and not identity_assessment.get("publishable"):
        ...
        if liveness == "inactive_domain":
            ...
            website_value["title"] = "Inactive domain — parked or under construction"
            website_value["description"] = f"Domain was inspected but is not an active company website: {reason_text}."
        elif liveness == "unverified_foreign_domain":
            website_value["title"] = "Foreign domain — ungrounded"
            website_value["description"] = "Domain uses a generic TLD and has no verifiable Norwegian company presence."
        elif liveness == "ambiguous_acronym":
            website_value["title"] = "Acronym collision — different entity"
            website_value["description"] = "Domain matches short acronym but belongs to a different registered business."
        else:
    ...
    wikidata_items = [
        item for item in external_observations
        if item.get("platform") == "wikidata" and item.get("signal_type") == "company_profile" and item.get("exact_entity")
    ]
    wikidata = wikidata_items[-1] if wikidata_items else None

    osm_items = [
        item for item in external_observations
        if item.get("platform") == "openstreetmap" and item.get("signal_type") == "place_summary" and item.get("exact_entity")
    ]
    osm = osm_items[-1] if osm_items else None
    ...
    site_news = [
        item for item in external_observations
        if item.get("platform") == "company_site" and item.get("signal_type") == "public_post" and item.get("exact_entity")
    ]
    hiring_signals = [
        item for item in external_observations
        if item.get("signal_type") in {"hiring_page_present", "job_posting"} and item.get("exact_entity")
    ]
    ...
    external = {
        ...
        "siteNews": [
            {
                "title": item.get("evidence_span") or "Company news / activity",
                "url": item.get("source_url"),
                "publishedAt": item.get("published_at") or item.get("retrieved_at"),
        ...
        "hiring": [
            {
                "title": item.get("evidence_span") or (item.get("metrics") or {}).get("job_title") or "Careers portal",
                "url": item.get("source_url"),
                "keywords": bool((item.get("metrics") or {}).get("hiring_keyword_detected")),
                **observation_meta(item),
            }
            for item in hiring_signals
        ],
        "wikidata": ({
            **(wikidata.get("metrics") or {}),
            "evidenceSpan": wikidata.get("evidence_span"),
            "url": wikidata.get("profile_url") or wikidata.get("source_url"),
            **observation_meta(wikidata),
        } if wikidata else None),
        "osm": ({
            **(osm.get("metrics") or {}),
            "evidenceSpan": osm.get("evidence_span"),
            "url": osm.get("profile_url") or osm.get("source_url"),
            **observation_meta(osm),
        } if osm else None),
        "linkedin": {
        ...
        "roles": {
            **meta(roles),
            "items": [
                {
                    **item,
                    "name": " ".join(item["name"]) if isinstance(item.get("name"), list) else item.get("name"),
                }
                for item in (roles.get("value") or {}).get("roles", [])[:30]
            ],
        },
...
<header class="mast"><div class="brand"><i></i>Signalpost laboratory</div><div class="snapshot">100-company showcase · Multi-layer verified intelligence</div></header>
...
function renderStats(){{const finance=DATA.filter(x=>available(x,'financial')).length,web=DATA.filter(x=>available(x,'web')&&x.web?.value?.identity_assessment?.publishable!==false).length,osm=DATA.filter(x=>x.external?.osm).length,wikidata=DATA.filter(x=>x.external?.wikidata).length,hiring=DATA.filter(x=>(x.external?.hiring||[]).length>0||(x.external?.linkedin?.jobs||[]).length>0).length,news=DATA.filter(x=>(x.external?.siteNews||[]).length>0||(x.external?.linkedin?.posts||[]).length>0).length;$('#index-total').textContent=DATA.length;$('#stats').innerHTML=[[DATA.length,'companies in showcase'],[finance,'annual accounts returned'],[web,'active company websites'],[osm,'OSM verified locations'],[wikidata,'Wikidata entities'],[hiring,'active hiring signals'],[news,'verified news & updates']].map(([v,l])=>`<div class="stat"><strong>${{v}}</strong><span>${{l}}</span></div>`).join('')}}
...
function requestedModes(question){{const q=norm(question),m=[];if(/financial|finance|account|revenue|income|profit|debt|asset|result|regnskap|inntekt|gjeld/.test(q))m.push('financials');if(/lead|leader|role|director|chair|board|ceo|leder|styre/.test(q))m.push('leaders');if(/location|where|address|shop|office|branch|place|sted|adresse|butikk|kontor|map|coord|lat|lon|osm|nominatim/.test(q))m.push('locations');if(/social|linkedin|facebook|instagram|youtube|tiktok|profile|handle/.test(q))m.push('social');if(/sentiment|reputation|opinion|positive|negative|omtale/.test(q))m.push('sentiment');if(/wiki|wikidata|wikipedia/.test(q))m.unshift('overview');if(/employee|company|overview|know|registered|industry|kommune|ansatt/.test(q)||!m.length)m.unshift('overview');return [...new Set(m)]}}
function agentBody(x,mode){{let body='';if(mode==='overview'){{const wiki=x.external?.wikidata;const wikiNote=wiki?`<br><strong>Wikidata:</strong> ${{esc(wiki.description||'Entity record')}} (<a href="${{esc(wiki.url)}}" target="_blank" rel="noreferrer">${{esc(wiki.qid)}} ↗</a>).`:'';const formatUrl=u=>!u?'':(/^https?:\\/\\//i.test(u)?u:'https://'+u);const wUrl=x.web.source||formatUrl(x.website);const webNote=wUrl?`<br><strong>Website:</strong> <a href="${{esc(wUrl)}}" target="_blank" rel="noreferrer">${{esc(wUrl)}} ↗</a>`:'';body=`<p><strong>${{esc(x.name)}}</strong> is registered as ${{esc(x.form||'form not reported')}} in ${{esc(x.municipality||'a municipality not reported in the snapshot')}}. Registry employees: ${{x.employees==null?'not reported':esc(x.employees)}}.${{cite(x.registrySource)}}${{webNote}}${{wikiNote}}</p>`}}if(mode==='financials'){{const r=x.financial.records[0];body=r?`<p>The latest normalized record reports revenue of <strong>${{money(r.revenue,r.currency)}}</strong>, operating result of <strong>${{money(r.operating_result,r.currency)}}</strong>, and debt of <strong>${{money(r.debt,r.currency)}}</strong>.${{cite(x.financial.source)}}</p>`:`<p>No normalized annual account was returned. Missing is not zero.${{cite(x.financial.source)}}</p>`}}if(mode==='leaders'){{const isService=r=>['REVI','REGN','FFØR'].includes(r.role_code)||/revisor|regnskap|forretningsf/i.test(r.role||r.group||'')||Boolean(r.organisation_number);const l=x.roles.items.filter(r=>!r.inactive&&!isService(r)),s=x.roles.items.filter(r=>!r.inactive&&isService(r));let text='';if(l.length){{text+=l.slice(0,5).map(r=>`<strong>${{esc(Array.isArray(r.name)?r.name.join(' '):(r.name||'Unnamed'))}}</strong> — ${{esc(r.role||r.group||'role')}}`).join('<br>')}}if(s.length){{text+=(text?'<br><small><strong>Auditors & accountants:</strong><br>':'')+s.slice(0,2).map(r=>`${{esc(Array.isArray(r.name)?r.name.join(' '):(r.name||'Service firm'))}} (${{esc(r.role||'Service')}})`).join('<br>')+(text?'</small>':'')}}body=text?`<p>${{text}}${{cite(x.roles.source)}}</p>`:`<p>No public role record was returned.${{cite(x.roles.source)}}</p>`}}if(mode==='locations'){{const osm=x.external?.osm;const osmNote=osm?`<br><br><strong>OpenStreetMap verified location:</strong> ${{esc(osm.display_name)}} (lat: ${{esc(osm.lat)}}, lon: ${{esc(osm.lon)}}). <a href="${{esc(osm.url)}}" target="_blank" rel="noreferrer">OSM ↗</a>`:'';body=x.locations.items.length?`<p>${{x.locations.items.slice(0,5).map(l=>`<strong>${{esc(l.name)}}</strong> — ${{esc(l.address?.kommune||l.address?.poststed||'address not reported')}}`).join('<br>')}}${{cite(x.locations.source)}}${{osmNote}}</p>`:(osmNote?`<p>No registered subunit was returned, but registered business address was verified on OpenStreetMap:${{osmNote}}</p>`:`<p>No registered subunit was returned. That does not prove there is no physical presence.${{cite(x.locations.source)}}</p>`)}}if(mode==='social')body=(x.web.value.social_links||[]).length?`<p>${{x.web.value.social_links.map(s=>`<strong>${{esc(s.platform)}}</strong> — <a href="${{esc(s.url)}}" target="_blank" rel="noreferrer">declared profile ↗</a>`).join('<br>')}}<br><small>Linked from the company-controlled site. Profile metrics have not yet passed the external audit.</small></p>`:x.web.value.quarantined_social_count?`<p><strong>${{esc(x.web.value.quarantined_social_count)}} social link(s) found but not published.</strong> The registry-linked site could not be verified as this exact legal entity.</p>`:`<p>No verified external profile data is available yet. A discovery task is planned; planned work is not evidence.</p>`;if(mode==='sentiment')body='<p><strong>Not available.</strong> External sentiment is now a scored competition module, but this snapshot has not passed the labelled Norwegian review/news audit.</p>';return body}}
...
function renderProfile(){{const x=selected;if(!x){{$('#profile').innerHTML='<p class="empty">Choose a company.</p>';renderAgent();return}}const rec=x.financial.records[0],people=x.roles.items.filter(r=>!r.inactive),locations=x.locations.items,social=x.web.value.social_links||[],pdfs=x.financialHistory.pdfs||[],isPinned=workspace.pins.includes(x.org),accounting=x.accounting.value||{{}};const formatUrl=u=>!u?'':(/^https?:\\/\\//i.test(u)?u:'https://'+u);const webUrl=x.web.source||formatUrl(x.website);const webPub=x.web.status==='available'&&x.web.value?.identity_assessment?.publishable!==false;const liveness=x.web.value?.identity_assessment?.liveness_status;let webBadgeCls=webPub?'good':'warn';let webBadgeText=webPub?'✓ Verified company website':'⚠ Registry link (unverified)';let webNoteText=webPub?'✓ Verified active company website':'⚠ Registry link (unverified)';if(!webPub&&liveness==='inactive_domain'){{webBadgeText='⚠ Inactive domain (parked / under construction)';webNoteText='⚠ Inactive domain (parked / under construction)';}}else if(!webPub&&liveness==='unverified_foreign_domain'){{webBadgeText='⊗ Foreign domain (ungrounded)';webNoteText='⊗ Foreign domain (ungrounded)';}}else if(!webPub&&liveness==='ambiguous_acronym'){{webBadgeText='⊗ Acronym collision';webNoteText='⊗ Acronym collision (different entity)';}}const displayWeb=(x.website||x.web.source||'').replace(/^https?:\\/\\//i,'').replace(/\\/$/,'');const isService=r=>['REVI','REGN','FFØR'].includes(r.role_code)||/revisor|regnskap|forretningsf/i.test(r.role||r.group||'')||Boolean(r.organisation_number);const leaders=people.filter(r=>!isService(r)),services=people.filter(r=>isService(r));$('#profile').innerHTML=`<div class="profile-head"><div><div class="eyebrow">${{esc(x.slice)}} sample · ${{esc(x.split)}}</div><h2>${{esc(x.name)}}</h2><div>${{esc(x.org)}} · ${{esc(x.form||'Form not reported')}}</div></div><div><button class="pin-button" id="pin-company" type="button">${{isPinned?'Unpin':'Pin company'}}</button><div class="chips" style="margin-top:8px">${{statusChip('accounts',x.financial.status)}}${{statusChip('roles',x.roles.status)}}${{statusChip('website',x.web.status)}}${{x.external?.osm?'<span class="chip good">OSM verified</span>':''}}${{x.external?.wikidata?'<span class="chip good">Wikidata</span>':''}}${{(x.external?.hiring||[]).length?'<span class="chip good">Hiring active</span>':''}}${{(x.external?.siteNews||[]).length?'<span class="chip good">News active</span>':''}}${{x.adverse?'<span class="chip warn">adverse registry state</span>':'<span class="chip good">no adverse flag</span>'}}</div></div></div><div class="fact-grid"><div class="fact"><span>Industry</span><strong>${{esc(x.industryCode||'—')}} ${{esc(x.industry||'Not reported')}}</strong></div><div class="fact"><span>Municipality</span><strong>${{esc(x.municipality||'Not reported')}}</strong></div><div class="fact"><span>Employees</span><strong>${{x.employees==null?'Not reported':esc(x.employees)}}</strong></div><div class="fact"><span>Official website</span>${{webUrl?`<strong><a href="${{esc(webUrl)}}" target="_blank" rel="noreferrer">${{esc(displayWeb)}} ↗</a></strong><small style="color:${{webPub?'var(--good)':'var(--warn)'}}">${{esc(webNoteText)}}</small>`:'<strong>Not reported</strong>'}}</div><div class="fact" style="grid-column:1 / -1"><span>Accounting availability</span><strong>${{esc((accounting.classification||'not classified').replaceAll('_',' '))}}</strong><small>${{esc(accounting.reason||'')}}</small></div></div><section><div class="section-label">Official annual account</div><h3>${{rec?'Latest normalized figures':'No normalized financial record returned'}}</h3>${{rec?`<table><tr><th>Period</th><td>${{esc(rec.period?.fraDato||'')}} – ${{esc(rec.period?.tilDato||'')}}</td></tr><tr><th>Revenue</th><td>${{money(rec.revenue,rec.currency)}}</td></tr><tr><th>Operating result</th><td>${{money(rec.operating_result,rec.currency)}}</td></tr><tr><th>Annual result</th><td>${{money(rec.annual_result,rec.currency)}}</td></tr><tr><th>Assets</th><td>${{money(rec.assets,rec.currency)}}</td></tr><tr><th>Debt</th><td>${{money(rec.debt,rec.currency)}}</td></tr></table>`:`<p class="empty">${{esc(accounting.reason||'Missing is not zero. Filing applicability is not established from the available source.') }}</p>`}}${{pdfs.length?`<p class="source">Historical filings: ${{pdfs.slice(0,8).map(p=>`<a href="${{esc(p.url)}}" target="_blank" rel="noreferrer">${{esc(p.year)}}</a>`).join(' · ')}}</p>`:''}}${{sourceLine(x.accounting)}}${{sourceLine(x.financial)}}</section><section><div class="section-label">Leadership &amp; Governance</div><h3>${{leaders.length}} board &amp; executive leaders</h3><div class="people">${{leaders.length?leaders.slice(0,16).map(r=>`<div class="item"><strong>${{esc(Array.isArray(r.name)?r.name.join(' '):(r.name||'Unnamed holder'))}}</strong><small>${{esc(r.role||r.group||'Role not reported')}}${{r.last_changed?` · Since ${{esc(r.last_changed)}}`:''}}</small></div>`).join(''):'<p class="empty">No board or executive role returned.</p>'}}</div>${{services.length?`<h3 style="margin-top:16px;font-size:17px;color:var(--muted)">Auditors &amp; accounting services (${{services.length}})</h3><div class="people">${{services.map(r=>`<div class="item" style="background:#faf8f5"><strong>${{esc(Array.isArray(r.name)?r.name.join(' '):(r.name||'Unnamed firm'))}}</strong><small>${{esc(r.role||r.group||'Service role')}}${{r.organisation_number?` · Org: ${{esc(r.organisation_number)}}`:''}}</small></div>`).join('')}}</div>`:''}}${{sourceLine(x.roles)}}</section><section><div class="section-label">Registered operating locations</div><h3>${{locations.length}} subunits${{x.external?.osm?' · OSM verified':''}}</h3>${{x.external?.osm?`<div class="item" style="grid-column:1 / -1;background:#f0f7f3;border-color:#9db9aa;margin-bottom:8px"><strong>OpenStreetMap verified location</strong><div>${{esc(x.external.osm.display_name||'Verified address match')}}</div><small>Coordinates: ${{esc(x.external.osm.lat)}}, ${{esc(x.external.osm.lon)}} · OSM: <a href="${{esc(x.external.osm.url)}}" target="_blank" rel="noreferrer">${{esc(x.external.osm.osm_type)}}/${{esc(x.external.osm.osm_id)}} ↗</a></small></div>`:''}}<div class="locations">${{locations.length?locations.slice(0,12).map(l=>`<div class="item"><strong>${{esc(l.name)}}</strong><small>${{esc(l.address?.kommune||l.address?.poststed||'Location not reported')}} · ${{l.employees??'employees not reported'}}</small></div>`).join(''):(x.external?.osm?'<p class="source">No distinct registered subunits in Brreg, but business address was verified on OpenStreetMap.</p>':'<p class="empty">Location module not run or no subunits returned.</p>')}}</div>${{sourceLine(x.locations)}}${{x.external?.osm?sourceLine(x.external.osm):''}}</section><section><div class="section-label">Update history</div><h3>${{x.changes.length}} recorded changes</h3>${{x.changes.length?`<table>${{x.changes.map(c=>`<tr><th>${{esc(c.field)}}</th><td>${{esc(JSON.stringify(c.old_value))}} → ${{esc(JSON.stringify(c.new_value))}}</td></tr>`).join('')}}</table>`:'<p class="empty">No changes are attached to this frozen profile yet. Retrieval timestamps and hashes below still show the evidence version.</p>'}}</section><section><div class="section-label">Company-controlled web layer</div>${{webUrl?`<div style="background:#f9f6f0;border:1px solid var(--line);padding:12px 14px;border-radius:4px;margin-bottom:14px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px"><div><span style="font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:var(--muted);display:block">Website URL</span><a href="${{esc(webUrl)}}" target="_blank" rel="noreferrer" style="font-size:15px;font-weight:600;word-break:break-all">${{esc(webUrl)}} ↗</a></div><span class="chip ${{webBadgeCls}}">${{esc(webBadgeText)}}</span></div>`:''}}<h3>${{esc(x.web.value.title||'No fetched homepage')}}</h3><p>${{esc(x.web.value.description||'No company-site description captured.')}}</p><p class="source">${{(x.web.value.pages||[]).length}} bounded same-site page(s) captured.</p><div class="social">${{social.map(s=>`<div class="item"><strong>${{esc(s.platform)}}</strong><small><a href="${{esc(s.url)}}" target="_blank" rel="noreferrer">${{esc(s.url)}}</a></small></div>`).join('')}}</div>${{sourceLine(x.web)}}</section>${{(x.external?.hiring||[]).length?`<section><div class="section-label">Careers &amp; Hiring signals</div><h3>${{x.external.hiring.length}} active career / vacancy signal(s)</h3><div class="people">${{x.external.hiring.slice(0,6).map(h=>`<div class="item"><strong>${{esc(h.title)}}</strong><small>${{h.keywords?'Active vacancy language detected':'Official career / job portal'}} · <a href="${{esc(h.url||h.source)}}" target="_blank" rel="noreferrer">view posting ↗</a></small></div>`).join('')}}</div>${{x.external.hiring[0]?.source?sourceLine(x.external.hiring[0]):''}}</section>`:''}}${{(x.external?.siteNews||[]).length?`<section><div class="section-label">Company news &amp; announcements</div><h3>${{x.external.siteNews.length}} verified news item(s)</h3><div class="activity-list">${{x.external.siteNews.slice(0,5).map(n=>`<div class="activity"><small>${{esc(n.publishedAt||'Recent')}}</small><p><strong>${{esc(n.title)}}</strong></p><small><a href="${{esc(n.url||n.source)}}" target="_blank" rel="noreferrer">read article ↗</a></small></div>`).join('')}}</div>${{x.external.siteNews[0]?.source?sourceLine(x.external.siteNews[0]):''}}</section>`:''}}${{x.external?.wikidata?`<section><div class="section-label">Knowledge graph footprint</div><h3>Wikidata &amp; Wikipedia identity</h3><div class="signal-card" style="background:#fcfbf9"><span>Wikidata entity</span><strong>${{esc(x.external.wikidata.item_label||x.name)}} (${{esc(x.external.wikidata.qid)}})</strong><p style="margin:6px 0">${{esc(x.external.wikidata.description||'Entity record verified against official Norwegian organisation number.')}}</p><p class="source"><a href="${{esc(x.external.wikidata.url||('https://www.wikidata.org/wiki/'+x.external.wikidata.qid))}}" target="_blank" rel="noreferrer">Wikidata ↗</a>${{x.external.wikidata.wikipedia_url?` · <a href="${{esc(x.external.wikidata.wikipedia_url)}}" target="_blank" rel="noreferrer">Wikipedia article ↗</a>`:''}}${{x.external.wikidata.sitelinks?` · ${{esc(x.external.wikidata.sitelinks)}} global sitelinks`:''}}</p></div>${{sourceLine(x.external.wikidata)}}</section>`:''}}`;$('#pin-company').onclick=()=>{{workspace.pins=isPinned?workspace.pins.filter(org=>org!==x.org):[...new Set([...workspace.pins,x.org])].sort();persistWorkspace();renderList();renderProfile()}};renderAgent();}}
...
const baseAgentBody=agentBody;agentBody=(x,mode)=>{{const li=x.external?.linkedin||{{}},profile=li.profile||{{}},jobs=li.jobs||[],posts=li.posts||[];const siteHiring=x.external?.hiring||[];const siteNews=x.external?.siteNews||[];if(mode==='social')return li.available?`<p><strong>LinkedIn:</strong> ${{profile.followers??'unreported'}} followers · ${{profile.visible_employees??'unreported'}} associated profiles · ${{esc(profile.employee_size_label||'size not reported')}}.${{cite(profile.source)}}</p>`:baseAgentBody(x,mode);if(mode==='hiring'){{let items=[];if(jobs.length)items.push(...jobs.slice(0,5).map(j=>`<strong>${{esc(j.title||'Role')}}</strong> — ${{esc(j.location||'location not reported')}} (<a href="${{esc(j.job_url||j.source)}}" target="_blank" rel="noreferrer">LinkedIn job ↗</a>)`));if(siteHiring.length)items.push(...siteHiring.slice(0,5).map(h=>`<strong>${{esc(h.title)}}</strong> — ${{h.keywords?'Active vacancy text detected':'Career portal'}} (<a href="${{esc(h.url||h.source)}}" target="_blank" rel="noreferrer">Careers ↗</a>)`));return items.length?`<p>${{items.join('<br>')}}</p>`:'<p>No exact-company LinkedIn job posting or verified company careers page was captured in this snapshot.</p>';}}if(mode==='activity'){{let acts=[];if(posts.length)acts.push(`<strong>${{posts.length}} LinkedIn post(s):</strong><br>`+posts.slice(0,2).map(p=>esc(p.text||'Post captured')).join('<br>'));if(siteNews.length)acts.push(`<strong>${{siteNews.length}} company news / announcement(s):</strong><br>`+siteNews.slice(0,3).map(n=>`• ${{esc(n.title)}} (<a href="${{esc(n.url||n.source)}}" target="_blank" rel="noreferrer">article ↗</a>)`).join('<br>'));return acts.length?`<p>${{acts.join('<br><br>')}}</p>`:'<p>No recent public LinkedIn posts or company site news updates captured in this snapshot.</p>';}}return baseAgentBody(x,mode)}};
...
if __name__ == "__main__":
    ...
    rows = [row.get("profile", row) for row in rows]
    if args.limit:
