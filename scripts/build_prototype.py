...
function renderStats(){{const finance=DATA.filter(x=>available(x,'financial')).length,web=DATA.filter(x=>available(x,'web')).length,osm=DATA.filter(x=>x.external?.osm).length,wikidata=DATA.filter(x=>x.external?.wikidata).length,hiring=DATA.filter(x=>(x.external?.hiring||[]).length>0||(x.external?.linkedin?.jobs||[]).length>0).length,news=DATA.filter(x=>(x.external?.siteNews||[]).length>0||(x.external?.linkedin?.posts||[]).length>0).length;$('#index-total').textContent=DATA.length;$('#stats').innerHTML=[[DATA.length,'companies in showcase'],[finance,'annual accounts returned'],[web,'company websites captured'],[osm,'OSM verified locations'],[wikidata,'Wikidata entities'],[hiring,'active hiring signals'],[news,'verified news & updates']].map(([v,l])=>`<div class="stat"><strong>${{v}}</strong><span>${{l}}</span></div>`).join('')}}
